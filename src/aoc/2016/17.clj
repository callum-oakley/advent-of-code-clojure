...
(defn adjacent [seed {[x y] :pos path :path}]
  (when (not= [3 3] [x y])
    (let [[a b] (hash/md5 (.getBytes (str seed path)))]
      (cond-> nil
        (and (pos? y) (< 10 (bit-and 0x0f (bit-shift-right a 4))))
        (conj {:pos [x (dec y)] :path (str path \U)})
        (and (< y 3) (< 10 (bit-and 0x0f a)))
        (conj {:pos [x (inc y)] :path (str path \D)})
        (and (pos? x) (< 10 (bit-and 0x0f (bit-shift-right b 4))))
        (conj {:pos [(dec x) y] :path (str path \L)})
        (and (< x 3) (< 10 (bit-and 0x0f b)))
        (conj {:pos [(inc x) y] :path (str path \R)})))))
