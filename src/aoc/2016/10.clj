  ...
  (reduce
   (fn [state line]
     (if-let [[_ value bot] (re-matches #"value (\d+) goes to (bot \d+)" line)]
       ...
       (let [[_ bot low high]
             (re-matches #"(bot \d+) gives low to (\w+ \d+) and high to (\w+ \d+)"
                         line)]
...
(defn part-1 []
  (-> "input/2016/10" slurp parse run (check-logs #{17 61})))

(defn part-2 []
  (apply * (-> "input/2016/10" slurp parse run (check-outputs [0 1 2]))))
